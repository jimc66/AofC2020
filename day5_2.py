...
def findseat(seatlist):
    ...
    find the number that is missing both the previous and next number
    return that integer, or negative one if no match
    """
    seatlist.sort() #put the list in order
    list_len = len(seatlist)
    iterator = 1
    while iterator < list_len - 1: #skip the last one
...
def main():
    ...
    reader.close
    high_seat_id = 0
    all_bording_passes = [] #for the list of all seat ids
#    tester = 'BBFFBBFRLL'
